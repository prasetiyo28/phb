<div id="content">
  ...
  <div class="section-action" style="webkit-box-shadow: none; box-shadow: none;">
    ...
    <div class="section-floating-action-row">
      <?php if ($this->session->userdata('bidang')==1) {
        $cbtn='btn-success';
      }elseif ($this->session->userdata('bidang')==2) {
        $cbtn='btn-info';
      }else {
        $cbtn='btn-warning';
      } ?>

      <div  class="btn-group" data-toggle="buttons" title="Data Produksi" style="top: -80px;left: -6px;">
				<label id="cb_mydatalabel" class="btn ink-reaction <?php echo $cbtn ?> btn-floating-action btn-md">
					<input type="checkbox" id="cb_mydata"> <i id="cb_mydataicon" class="fa fa-archive"></i>
				</label>
		  </div>
    </div>
    <div id="btn_my_menu" style="display:none;">
      <div id="btn_my_menu1" class="section-floating-action-row">
        <div class="btn-group" data-toggle="buttons" title="Panen" style="top: -130px;left: -12px;">
          <label id="cb_mydatalabel_panen" class="btn ink-reaction btn-accent btn-floating-action btn-sm">
            <input type="checkbox" id="cb_mydata_panen"> <i id="cb_mydataicon_panen" class="md md-done-all"></i>
          </label>
        </div>
      </div>
      <div id="btn_my_menu2" class="section-floating-action-row">
        <div class="btn-group" data-toggle="buttons" title="Belum Panen" style="top: -170px;left: -12px;">
          <label id="cb_mydatalabel_belum" class="btn ink-reaction btn-info btn-floating-action btn-sm">
            <input type="checkbox" id="cb_mydata_belum"> <i id="cb_mydataicon_belum" class="md md-restore"></i>
          </label>
        </div>
      </div>
      <div id="btn_my_menu3" class="section-floating-action-row">
        <div class="btn-group" data-toggle="buttons" title="Gagal Panen" style="top: -210px;left: -12px;">
          <label id="cb_mydatalabel_gagal" class="btn ink-reaction btn-danger btn-floating-action btn-sm">
            <input type="checkbox" id="cb_mydata_gagal"> <i id="cb_mydataicon_gagal" class="md md-highlight-remove"></i>
          </label>
        </div>
      </div>
    </div>
...
<?php $this->load->view('Admin/peta/modal_produksi'); ?>
<div id="viewcanvas"></div>
<?php $this->load->view('Admin/pengguna/modal_peta') ?>
...
function markerLocation1(){
  var geocoder = new google.maps.Geocoder;
    ...
    $("#form_peta").removeAttr("action");
    $("#form_peta").attr('action',"<?php echo base_url('Admin/update_peta_produksi'); ?>");
...
$(function() {
  ...
  var get_mark8=false;
  var get_mark9=false;
  var get_mark_my_panen=false;
  var get_mark_my_belum=false;
  var get_mark_my_gagal=false;
    ...
    if ($('#cb_pertanian_panen').is(':checked')) {
      if (get_mark1==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_pertanian_panen'); ?>"
          }
        ).done(function( data )
        {
    ...
    if ($('#cb_pertanian_belum').is(':checked')) {
      if (get_mark2==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_pertanian_belum'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
    ...
    if ($('#cb_pertanian_gagal').is(':checked')) {
      if (get_mark3==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_pertanian_gagal'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
    ...
    if ($('#cb_perikanan_panen').is(':checked')) {
      if (get_mark4==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_perikanan_panen'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
    ...
    if ($('#cb_perikanan_belum').is(':checked')) {
      if (get_mark5==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_perikanan_belum'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
    ...
    if ($('#cb_perikanan_gagal').is(':checked')) {
      if (get_mark6==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_perikanan_gagal'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
    ...
    if ($('#cb_peternakan_panen').is(':checked')) {
      if (get_mark7==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_peternakan_panen'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
    ...
    if ($('#cb_peternakan_belum').is(':checked')) {
      if (get_mark8==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_peternakan_belum'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
    ...
    if ($('#cb_peternakan_gagal').is(':checked')) {
      if (get_mark9==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_peternakan_gagal'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
  ...
  $('#cb_mydatalabel_panen').click(function() {
    console.log($('#cb_mydata_panen').val());
    if ($('#cb_mydata_panen').is(':checked')) {
      if (get_mark_my_panen==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_mydata_panen'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
          ...
          for (var i = 0; i < count; i++) {
           ...
           var sContent = '<div class="iw-container">' +
                  ...
                  ' (kg/ekor)</td></tr><tr><td>Harga Perkiraan Perkilo/Ekor</td><td>Rp. '+jsonObj[i].harga_kira_perkilo+'</td></tr></tbody></table>' +	'</div>'+
                  '</div>'+'</div>';
            addMarker(lat_lng,"10",jsonObj[i].icon,sContent);
          }
          get_mark_my_panen=true;
        });
        $('#cb_mydatalabel_panen').removeClass('btn-accent');
        $('#cb_mydatalabel_panen').addClass('btn-danger');
        $('#cb_mydataicon_panen').removeClass('md md-done-all');
        $('#cb_mydataicon_panen').addClass('md md-close');
      ...
      deleteMarkers('10');
      $('#cb_mydatalabel_panen').removeClass('btn-danger');
      $('#cb_mydataicon_panen').removeClass('md md-close');
      $('#cb_mydataicon_panen').addClass('md md-done-all');
      $('#cb_mydatalabel_panen').addClass('btn-accent');
    }
  });
  $('#cb_mydatalabel_belum').click(function() {
    console.log($('#cb_mydata_belum').val());
    if ($('#cb_mydata_belum').is(':checked')) {
      if (get_mark_my_belum==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_mydata_belum'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
          ...
          for (var i = 0; i < count; i++) {
           ...
           var sContent = '<div class="iw-container">' +
                  ...
                  ' (kg/ekor)</td></tr><tr><td>Harga Perkiraan Perkilo/Ekor</td><td>Rp. '+jsonObj[i].harga_kira_perkilo+'</td></tr></tbody></table>' +	'</div>'+
                  '</div>'+'</div>';
            addMarker(lat_lng,"11",jsonObj[i].icon,sContent);
          }
          get_mark_my_belum=true;
        });
        $('#cb_mydatalabel_belum').removeClass('btn-info');
        $('#cb_mydatalabel_belum').addClass('btn-danger');
        $('#cb_mydataicon_belum').removeClass('md md-restore');
        $('#cb_mydataicon_belum').addClass('md md-close');
      ...
      deleteMarkers('11');
      $('#cb_mydatalabel_belum').removeClass('btn-danger');
      $('#cb_mydataicon_belum').removeClass('md md-close');
      $('#cb_mydataicon_belum').addClass('md md-restore');
      $('#cb_mydatalabel_belum').addClass('btn-info');


    }
  });
  $('#cb_mydatalabel_gagal').click(function() {
    console.log($('#cb_mydata_gagal').val());
    if ($('#cb_mydata_gagal').is(':checked')) {
      if (get_mark_my_gagal==false) {
        $.ajax(
          {
            type: "GET",
            url: "<?php echo base_url('Admin/get_all_mydata_gagal'); ?>"
          }
        ).done(function( data )
        {
          var jsonObj = JSON.parse(data);
          ...
          for (var i = 0; i < count; i++) {
           ...
           var sContent = '<div class="iw-container">' +
                  ...
                  ' (kg/ekor)</td></tr><tr><td>Harga Perkiraan Perkilo/Ekor</td><td>Rp. '+jsonObj[i].harga_kira_perkilo+'</td></tr></tbody></table>' +	'</div>'+
                  '</div>'+'</div>';
            addMarker(lat_lng,"12",jsonObj[i].icon,sContent);
          }
          get_mark_my_gagal=true;
        });
        $('#cb_mydatalabel_gagal').removeClass('btn-info');
        $('#cb_mydatalabel_gagal').addClass('btn-danger');
        $('#cb_mydataicon_gagal').removeClass('md md-highlight-remove');
        $('#cb_mydataicon_gagal').addClass('md md-close');
      ...
      deleteMarkers('12');
      $('#cb_mydatalabel_gagal').removeClass('btn-danger');
      $('#cb_mydataicon_gagal').removeClass('md md-close');
      $('#cb_mydataicon_gagal').addClass('md md-highlight-remove');
      $('#cb_mydatalabel_gagal').addClass('btn-danger');
  ...
  $('#cb_mydatalabel').click(function() {
    console.log($('#cb_mydata').val());
    if ($('#cb_mydata').is(':checked')) {
        $('#cb_mydatalabel').removeClass('btn-success btn-info btn-warning');
        $('#cb_mydatalabel').addClass('btn-danger');
        $('#cb_mydataicon').removeClass('fa fa-archive');
        $('#cb_mydataicon').addClass('md md-close');
         $('#btn_my_menu').show('slow');
        $('#btn_my_menu1').animate({ top: "+=50" }, "fast" ).fadeIn()
                   .animate({ top: '-=50' }, "fast" );
        $('#btn_my_menu2').delay(300).animate({ top: "+=50" }, "fast" ).fadeIn()
                   .animate({ top: '-=50' }, "fast" );
        $('#btn_my_menu3').delay(600).animate({ top: "+=50" }, "fast" ).fadeIn()
                   .animate({ top: '-=50' }, "fast" );

    }else {
      $('#cb_mydatalabel').removeClass('btn-danger');
      $('#cb_mydataicon').removeClass('md md-close');
      $('#cb_mydataicon').addClass('fa fa-archive');

      <?php if ($this->session->userdata('bidang')==1): ?>
      $('#cb_mydatalabel').addClass('btn-success');
      <?php elseif ($this->session->userdata('bidang')==2): ?>
      $('#cb_mydatalabel').addClass('btn-info');
      <?php else: ?>
      $('#cb_mydatalabel').addClass('btn-warning');
      <?php endif; ?>
      $('#btn_my_menu').hide('fast');
    }
  });
...
function deleteAct(id) {
  var data = new FormData();
    ...
    var url = "<?php echo base_url('Admin/deleteProduksi/'); ?>";
...
function edit(id) {
  ...
  $.ajax(
    {
      type: "GET",
      url: "<?php echo base_url('Admin/edit_produksi'); ?>/"+id,
    }
  ).done(function( data )
  {
...
function detail(id) {
  ...
  $.ajax(
    {
      type: "GET",
      url: "<?php echo base_url('Admin/detail_produksi'); ?>/"+id,
    }
  ).done(function( data )
  {
    $('#viewcanvas-kiri').html(data);
...
function profil(id) {
  ...
  $.ajax({
            ...
            processData: false,
            contentType: false,
            url:"<?php echo base_url('Admin/makesesion/') ?>",
            success: function(data)
            {
              if (data=='1') {
