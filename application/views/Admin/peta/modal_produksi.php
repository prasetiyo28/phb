<div id="modal_add_produksi" class="modal fade" role="dialog">
  <div class="modal-dialog modal-lg">
    <!-- Modal content-->
    <form class="form form-validate" id="form_produksi" novalidate="novalidate" method="post" action="<?php echo base_url() ?>/Admin/insert_produksi">
				<div class="card">
            ...
            <div class="row">
              ...
              <div class="col-sm-6">
                <div class="form-group">
                  <input type="text" class="form-control" name="nik" disabled>
                  <label>NIK</label>
                </div>
              </div>
            </div>
...
$(function() {
  ...
  $.ajax({
  url: "<?php echo base_url('Admin/get_all_user_ajax') ?>",
  dataType: "json",
  success: function (data) {
    minLength: 2,
    $('[name="nama"]').autocomplete({
      source: function (request, response) {
        var results = $.ui.autocomplete.filter(data.nama, request.term);
        response(results.slice(0, 10));
      },
      select: function( event, ui ) {
        var indek = $.inArray(ui.item.value, data.nama);
        $('[name="id_user"]').val(data.id_user[indek]);
        $('[name="nik"]').val(data.nik[indek]);
        if (data.bidang[indek]=='1') {
