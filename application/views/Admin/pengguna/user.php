<div class="margin-bottom-xxl">
  <span class="text-light text-lg">Total Pengguna <strong><?php echo total_user_foradmin() ?></strong></span>
...
$(function () {
  ...
  $('#btn_cek').click(function() {
    ...
    if (nik!='' && pjg_nik >= 16) {
      ...
      $.ajax(
        {
          type: "GET",
          url: "<?php echo base_url('Admin/cek_nik'); ?>/"+nik,
          dataType:"JSON"
        }
      ).done(function( data )
      {
        if (data.result==1) {
          ...
          $.ajax({
            type:"GET",
            url: "<?php echo base_url('Admin/api_nik'); ?>/"+nik,
            dataType:"JSON"
          }).done(function(data) {
            if (data.result==1) {
              $('#got_people').show();
      ...
      $('#cari_nik').addClass('form-group has-error has-feedback');
      $('#cari_nik_icon').addClass('glyphicon glyphicon-remove form-control-feedback');
      $('#cari_nik_p').text('Isian ini wajib di isi!');
      $('#btn_clear').text('BERSIHKAN');
      $('.preload1').hide();
    }
  ...
  function loadData(sort,find,find_value) {
    ...
    window.pagObj = $('#test-list').twbsPagination({
        totalPages: "<?php echo total_page_foradmin() ?>",
        visiblePages: 5,
        ...
        onPageClick: function (event, page) {
            ...
            $.ajax(
              {
                type: "GET",
                url: "<?php echo base_url('Admin/user_data'); ?>/"+find+"/"+find_value+"/"+sort+"/"+id,
              }
            ).done(function( data )
            {
              $('#list').html(data);
