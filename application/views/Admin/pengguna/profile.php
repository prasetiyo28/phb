						...
						<div class="overlay overlay-shade-bottom stick-bottom-left force-padding text-right">
							...
							<div class="btn-group">
								...
								<ul class="dropdown-menu animation-slide" role="menu" style="right:0;left:unset;">
									...
									<li class="divider"></li>
									<?php if ($blokir=="1"): ?>
										<li><a href="<?php echo base_url('Admin/unblokir_user').'/'.$id?>"><i class="fa fa-check text-danger"></i> Aktifkan</a></li>
									<?php else: ?>
										<li><a href="<?php echo base_url('Admin/blokir_user').'/'.$id?>"><i class="fa fa-ban text-danger"></i> Blokir</a></li>
								<?php endif; ?>
									<li><a href="<?php echo base_url('Admin/hapus_user').'/'.$id?>"><i class="fa fa-trash text-danger"></i> Hapus</a></li>
								</ul>
							</div>
							</div>
					...
					<div class="section-body no-margin">
						<div class="row">
							...
							<div class="col-lg-offset-1 col-lg-3 col-md-4">
								<?php foreach ($data as $d): ?>
									<form action=" <?php echo base_url('Admin/update_personal') ?>" class="form form-validate" novalidate="novalidate" method="post">
...
$(document).ready(function(){
  ...
  $(function() {
	...
	function modal_ktp(foto,id,n,par) {
			$.ajax({
				type:"GET",
				url: "<?php echo base_url('Admin/modal_ktp'); ?>/"+foto+"/"+id+"/"+n+"/"+par,
			}).done(function(data) {
				$('#viewmodal').html(data);
	...
	function cari_lap() {
		...
		$.ajax(
			{
				type: "GET",
				url: "<?php echo base_url('Admin/cari_lap_user'); ?>/"+jenis2+"/"+bulan+"/"+tahun,
			}
		).done(function( data )
		{
			$('#viewlap').html(data);
