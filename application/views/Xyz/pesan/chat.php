      ...
      <div class="row">
        ...
        <div class="col-sm-4 col-md-3 col-lg-2">
          ...
          <ul class="nav nav-pills nav-stacked nav-icon">
            ...
            <li id="lic" class="active"><a  href="<?php echo base_url('Xyz/chat') ?>"><span class="glyphicon glyphicon-inbox"></span>Chatting <small id="chatNotif"></small></a></li>
...
$(document).ready(function() {
  ...
  $("#form_chat").validate();
    ...
    $.ajax(
      {
        type: "GET",
        url: "<?php echo base_url('Xyz/notifChat/'); ?>"
      }
    ).done(function( data )
    {
      ...
      if (count!=0) {
        ...
        toastr.info(count+" Pesan diterima", "");
...
function send() {
  ...
  if(!valid) {
    ...
    $.ajax({
      ...
      processData: false,
      contentType: false,
      url: "<?php echo base_url('Xyz/send_chat')?>",
...
function viewchat(room_id,id_chat) {
  $.ajax(
    {
      type: "GET",
      url: "<?php echo base_url('Xyz/readChat/'); ?>"+room_id+"/"+id_chat
    }
  ).done(function( data )
  {
    $('#box_chat').html(data);
...
function deleteAct(room_id) {
  ...
  $.ajax(
    {
      type: "POST",
      url: "<?php echo base_url('Xyz/deleteChat/'); ?>",
      data: data,
      processData: false,
      contentType: false,
