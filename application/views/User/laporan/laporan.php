...
<?php echo $this->session->userdata('pekerjaan') ?>
      <center>
      <a href="<?php echo base_url('User/print_laporan/') ?>" target="_blank" type="button" class="btn ink-reaction btn-floating-action btn-primary"><i class="fa fa-print"></i></a>
    </center>
      <br/>
...
function cari_lap_indeks() {
  ...
  if (icon!='') {
    $.ajax(
      {
        type: "GET",
        url: "<?php echo base_url('User/cari_lap_indeks'); ?>/"+icon+"/"+jenis+"/"+tahun,
      }
    ).done(function( data )
    {
...
function cari_lap_pericon() {
  ...
  $.ajax(
    {
      type: "GET",
      url: "<?php echo base_url('User/cari_lap_pericon'); ?>/"+icon+"/"+tahun,
    }
  ).done(function( data )
  {
