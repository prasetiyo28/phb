...
public function get_all_select_produksi($id)
{
	return $this->db->select('*')
	->from('tb_user')
	->join('tb_produksi','tb_user.id_user = tb_produksi.id_user')
	->where('tb_user.del_flag','1')
	->where('tb_produksi.panen_flag','0')
	->where('tb_user.id_user',$id)
	->get();
}
...
{
	return $this->db->select('sum(pengeluaran) as pengeluaran')
	->where('tb_catatan.id_produksi',$id)
	->group_by('tb_catatan.id_produksi')
	->get('tb_catatan');
}
...
public function hadiah_point($pekerjaan,$id_kab)
{
	return $this->db->select('tb_admin.*,tb_hadiah_point.*')
	->from('tb_admin')
	->join('tb_hadiah_point','tb_admin.id_admin = tb_hadiah_point.id_admin')
	->where('tb_hadiah_point.del_flag','1')
	->where('tb_admin.bidang',$pekerjaan)
	->where('tb_admin.id_kab',$id_kab)
	->get();
}
}
?>
