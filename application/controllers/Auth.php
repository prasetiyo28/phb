...
class Auth extends CI_Controller {
  ...
  public function login()
  {
    ...
    $row4 = $this->Auth_model->cek("tb_wirausahawan",$where4)->row();
      ...
      if ($remember) {
        $key = random_string('alnum', 64);
            ...
            // 1. Buat Cookies jika remember di check
              ...
              $this->_daftarkan_session_wirausahawan($row4);
            } 
        ...
        // 2. Redirect ke home
          ...
          public function _daftarkan_session_wirausahawan($row) {
        // 1. Daftarkan Session
            $sess = array(
              'logged' => TRUE,
              'id' => $row->id_wirausahawan,
              'nama' => $row->nama,
              'toko' => $row->toko,
              'email' => $row->email,
              'level' => 'wirausahawan',
